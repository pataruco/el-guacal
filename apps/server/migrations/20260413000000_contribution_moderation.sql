...
CREATE OR REPLACE TRIGGER set_updated_at_users BEFORE UPDATE ON users
...
CREATE OR REPLACE TRIGGER set_updated_at_store_proposals BEFORE UPDATE ON store_proposals
FOR EACH ROW EXECUTE PROCEDURE set_updated_at();

CREATE INDEX IF NOT EXISTS idx_store_proposals_status ON store_proposals(status);
CREATE INDEX IF NOT EXISTS idx_store_proposals_proposer ON store_proposals(proposer_user_id);
CREATE INDEX IF NOT EXISTS idx_store_proposals_location ON store_proposals USING GIST (proposed_location);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='stores' AND COLUMN_NAME='version') THEN
